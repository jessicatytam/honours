...
#model
mod_list2_10 <- vector(mode = "list", length = 100)
for (i in 1:length(random_trees)) {
  for (j in 1:length(imp_10)) {
    #mod_list2_10[[j+(i-1)*length(imp_10)]] <- sprintf("%s is i and %s is j\n", i, j)
    mod_list2_10[[j+(i-1)*length(imp_10)]] <- MCMCglmm(h ~ logmass +
                                                       abs_lat +
                                                       humanuse_bin +
                                                       domestication_bin +
                                                       poly(iucn_bin, 2) +
...
#save the models
saveRDS(mod_list2_10, "data/intermediate_data/MCMCglmm/mod_list2_10.rds")
