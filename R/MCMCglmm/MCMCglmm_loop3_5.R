...
#model
mod_list3_5 <- vector(mode = "list", length = 100)
for (i in 1:length(random_trees)) {
  for (j in 1:length(imp_5)) {
    #mod_list3_5[[j+(i-1)*length(imp_5)]] <- sprintf("%s is i and %s is j\n", i, j)
    mod_list3_5[[j+(i-1)*length(imp_5)]] <- MCMCglmm(h ~ logmass +
                                                       abs_lat +
                                                       humanuse_bin +
                                                       poly(iucn_bin, 2) +
                                                       log_sumgtrends,
...
#save the models
saveRDS(mod_list3_5, "data/intermediate_data/MCMCglmm/mod_list3_5.rds")
