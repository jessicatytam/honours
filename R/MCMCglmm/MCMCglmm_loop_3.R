...
#model
mod_list_3 <- vector(mode = "list", length = 100)
for (i in 1:length(random_trees)) {
  for (j in 1:length(imp_3)) {
    #mod_list_3[[j+(i-1)*length(imp_3)]] <- sprintf("%s is i and %s is j\n", i, j)
    mod_list_3[[j+(i-1)*length(imp_3)]] <- MCMCglmm(h ~ logmass +
                                                    abs_lat +
                                                    humanuse_bin +
                                                    domestication_bin +
                                                    iucn_bin +
...
#save the models
saveRDS(mod_list_3, "data/intermediate_data/MCMCglmm/mod_list_3.rds")
